...
export default function ClientesProveedores() {
  ...
  React.useEffect(() => {
    const getData = async (setState, state) => {
      const api = await Axios.get("http://18.228.152.164/api/cajachica");
  ...
  function postAddRow(data) {
    console.log(data)
    Axios.post("http://18.228.152.164/api/cajachica", data)
      .then(response => null)
      .catch(error => {
  ...
  function postUpdateRow(data){const id = data["_id"]["$oid"]
    ...
    Axios.post(`http://18.228.152.164/api/cajachica/${id}`, data)
    .then(response => null)
    .catch(error => {
    console.error('There was an error!', error);
     });
  ...
  function postDeleteRow(id) {
    Axios.delete(`http://18.228.152.164/api/cajachica/${id}`);
  };
